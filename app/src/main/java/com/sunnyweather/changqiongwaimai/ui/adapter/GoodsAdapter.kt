...
class GoodsAdapter(
    ...
    private val onAddCartClicked: (goodsId: Int, selectedSpicyLevel: String) -> Unit,  //添加到购物车回调
    private val onIncrease: (Goods) -> Unit,    //商品加号回调
    private val onDecrease: (Goods) -> Unit    //商品减号回调
) :
    ...
    fun submitList(newList: List<Goods>) {
        goodsList = newList
        //让 RecyclerView 重新刷新所有列表项，包括已经渲染的和未渲染的项。
    ...
    private fun showCenterDialog(price: Double, id: Int) {
        ...
        //_ 在 Kotlin 中是一种 “忽略参数” 的语法。
        //当一个回调或函数有多个参数，而你 不需要使用其中某些参数 时，就可以用 _ 来替代。
